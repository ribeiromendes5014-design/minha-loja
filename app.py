...
# =====================================
# Funções auxiliares (definidas antes da execução)
# =====================================
    ...
    def abrir_caixa():
        with st.form("abrir_caixa_form"):
            ...
            if not caixa_hoje.empty and caixa_hoje.iloc[0]["Status"] == "Aberto":
                ...
                st.session_state["valor_inicial"] = float(caixa_hoje.iloc[0].get("ValorInicial", 0.0))
        ...
        if "caixa_aberto" in st.session_state and st.session_state["caixa_aberto"]:
            ...
            if st.button("📦 Confirmar Fechamento de Caixa", key="btn_confirmar_fechar_caixa"):
                dados_caixa = {
                    ...
                    "Cartão": total_cartao,
                    "Fiado": total_fiado,
                    "ValorFinalInformado": valor_final_informado,
                    "Status": "Fechado"
                }
                caixas = norm_caixas(pd.DataFrame())
                caixas_index = caixas[caixas["Data"] == hoje].index
                if not caixas_index.empty:
                    caixas.loc[caixas_index[0], "Status"] = "Fechado"
                    caixas.loc[caixas_index[0], "ValorFinalInformado"] = valor_final_informado
                    caixas.loc[caixas_index[0], "Operador"] = operador
                    caixas.loc[caixas_index[0], "ValorInicial"] = valor_inicial
                else:
