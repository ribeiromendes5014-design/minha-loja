...
# =====================================
def _barcode_recv(frame):
    """Processor para streamlit-webrtc: detecta códigos e grava em session_state['codigo_barras']."""
    ...
    for bc in barcodes:
        ...
        st.session_state["codigo_barras"] = code
        if cv2 is not None:
            (x, y, w, h) = bc.rect
            cv2.rectangle(img, (x, y), (x+w, y+h), (0, 255, 0), 2)
            cv2.putText(img, code, (x, y - 8), getattr(cv2, "FONT_HERSHEY_SIMPLEX", 0), 0.7, (0, 255, 0), 2)
    return av.VideoFrame.from_ndarray(img, format="bgr24")

def barcode_scanner_ui(key: str = "barcode"):
    """Mostra o componente de câmera e mantém último código em st.session_state['codigo_barras']"""
    webrtc_streamer(
        key=key,
        mode=WebRtcMode.SENDRECV,
        video_processor_factory=lambda: type("BarcodeProcessor", (), {"recv": _barcode_recv})(),
...
def boot_session():
    ...
    st.session_state.setdefault("codigo_barras", "")
...
if view == "Produtos":
    ...
    with st.expander("Cadastrar novo produto"):
        ...
        barcode_scanner_ui("barcode_cad")

        codigo_barras = st.text_input("Código de Barras (ID do produto)", value=st.session_state.get("codigo_barras",""))
        ...
        preco_cartao_calc = 0.0
        try:
            preco_cartao_calc = round(float(preco_vista.replace(",", ".").strip()) / FATOR_CARTAO, 2)
        except Exception:
            preco_cartao_calc = 0.0
        ...
        if st.button("Adicionar produto"):
            if not str(codigo_barras).strip():
                st.error("Escaneie ou insira manualmente o código de barras (será usado como ID).")
            else:
                # Se já existir, atualiza; senão cria novo
                mask = produtos["ID"].astype(str) == str(codigo_barras).strip()
                novo = {
                    "ID": str(codigo_barras).strip(),
                    "Nome": nome.strip(),
                    "Marca": marca.strip(),
                ...
                if mask.any():
                    produtos.loc[mask, list(novo.keys())] = list(novo.values())
                    st.info("Produto existente atualizado pelo mesmo código de barras.")
                else:
    ...
    if produtos.empty:
        ...
        for _, row in produtos.iterrows():
            with st.container():
                ...
                if col_btn.button("🗑️ Excluir", key=f"del_{eid}"):
                    produtos = produtos[produtos["ID"].astype(str) != str(eid)]
                    st.session_state["produtos"] = produtos
                    save_csv(produtos, ARQ_PRODUTOS)
        ...
        if "edit_prod" in st.session_state:
            ...
            row = produtos[produtos["ID"].astype(str)==str(eid)]
            if not row.empty:
                st.subheader(f"Editar produto (ID/Código: {eid})")
                row = row.iloc[0]
                c1,c2,c3 = st.columns(3)
                with c1:
                    novo_id = st.text_input("Código de Barras (ID)", value=str(row["ID"]))  # permite trocar o código
                ...
                if st.button("Salvar alterações"):
                    # Se o ID mudou, precisamos atualizar a chave
                    mudou_id = str(novo_id).strip() != str(eid)
                    if mudou_id and (produtos["ID"].astype(str) == str(novo_id).strip()).any():
                        st.error("Já existe um produto com esse código de barras (ID). Escolha outro.")
                    else:
                        idx_mask = produtos["ID"].astype(str)==str(eid)
                        produtos.loc[idx_mask, ["ID","Nome","Marca","Categoria","Quantidade","PrecoCusto","PrecoVista","PrecoCartao","Validade","FotoURL"]] = [
                            str(novo_id).strip(),
                            novo_nome.strip(),
                            nova_marca.strip(),
                            nova_cat.strip(),
...
# =====================================
# VENDAS (Layout completo com promoções + leitor de código)
# =====================================
if view == "Vendas":
    show_logo("main")
    ...
    # -- Leitor de código de barras para preencher o campo "Código do Produto"
    st.subheader("Escanear código de barras")
    barcode_scanner_ui("barcode_vendas")
    st.caption("Ao detectar, o código preenche o campo abaixo automaticamente.")

    # -- Seleção de produto
    st.subheader("Itens do pedido")
    c1,c2,c3,c4 = st.columns([2,3,2,2])
    with c1:
        codigo = st.text_input("Código do Produto", value=st.session_state.get("codigo_barras",""))
    with c2:
    ...
    with col_preco:
        if escolha is not None:
            ...
            if not rowp.empty:
                rowp = rowp.iloc[0]
                preco_vista_val = float(rowp["PrecoVista"])

                # Aplica promoção (se houver) para mostrar ao usuário
                preco_vista_aplicado, promo = preco_vista_com_promocao(pid, preco_vista_val, date.today(), promocoes)
                preco_unit = preco_por_forma(preco_vista_aplicado, forma)

                if promo:
                    st.info(f"🏷️ Promoção ativa: -{promo['Desconto']:.0f}% até {promo['DataFim']}. Preço à vista de {brl(preco_vista_val)} por {brl(preco_vista_aplicado)}.")
                ...
                if st.button("Adicionar ao pedido"):
                    st.session_state["pedido_atual"].append({
                        ...
                        "Quantidade": int(qtd),
                        "PrecoVista": preco_vista_val,  # guardo o à vista base; a promoção será recalculada ao final
                    })
    ...
    if promocoes.empty:
        ...
        if sel:
            ...
            if not linha.empty:
                ...
                with col3:
                    try:
                        df_ = parse_date_yyyy_mm_dd(ln["DataFim"]) or (date.today()+timedelta(days=7))
                    except Exception:
                        df_ = date.today()+timedelta(days=7)
                    data_fim_e = st.date_input("Término", value=df_, key=f"edit_fim_{sel}")
