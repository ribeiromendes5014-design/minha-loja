...
def garantir_colunas_clientes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in cols:
        if c not in df.columns:
            df[c] = None
    df["Valor"] = pd.to_numeric(df.get("Valor"), errors="coerce").fillna(0.0).astype(float)
    df["DataPagamento"] = pd.to_datetime(df.get("DataPagamento"), errors="coerce").dt.date
    # Status padrão "Aberto" quando vazio
    if "Status" in df.columns:
        df["Status"] = df["Status"].fillna("Aberto").replace({"": "Aberto"})
    else:
        df["Status"] = "Aberto"
    df["DataPagamento"] = df["DataPagamento"].astype(str)
    return df[cols].reset_index(drop=True)
...
# =============================
    ...
    if produtos.empty:
        ...
        nome_cliente_fiado, data_pagamento_fiado = None, None
        if forma == "Fiado":
        ...
        if st.button("Registrar Venda"):
            if qtd > int(p["Quantidade"]):
                ...
                # se venda for fiado, registrar em clientes
                if forma == "Fiado":
                    novo_cli = {
                        "ID": prox_id(clientes, "ID"),
                        "Cliente": (nome_cliente_fiado or "").strip(),
                        "Produto": str(p["Nome"]),
                        "Valor": float(total),
                        "DataPagamento": (data_pagamento_fiado or date.today()),
                        "Status": "Aberto"
                    }
...
# =============================
    ...
    # Formulário para adicionar fiado
    ...
    if clientes.empty:
        ...
        # Garantir coluna Status e separar abertas
        if "Status" not in clientes.columns:
            clientes["Status"] = "Aberto"
        abertos = clientes[clientes["Status"] == "Aberto"].copy()
        st.write("**Em aberto:**")
        st.dataframe(abertos, use_container_width=True)

        # Ações de gestão
        if not abertos.empty:
            selecoes = [f'{int(r.ID)} - {r.Cliente} | {r.Produto} | R$ {float(r.Valor):.2f} | vence {r.DataPagamento}' for r in abertos.itertuples(index=False)]
            esco = st.selectbox("Selecione a dívida", selecoes)
            sel_id = int(esco.split(" - ")[0])
            colA, colB = st.columns(2)
            with colA:
                if st.button("💰 Quitar dívida"):
                    clientes.loc[clientes["ID"] == sel_id, "Status"] = "Pago"
                    salvar_csv(clientes, ARQ_CLIENTES)
                    st.success("Dívida marcada como paga.")
            with colB:
                if st.button("🗑️ Excluir dívida"):
                    clientes = clientes[clientes["ID"] != sel_id].reset_index(drop=True)
                    salvar_csv(clientes, ARQ_CLIENTES)
        ...
        vencidas = clientes[(clientes["Status"] == "Aberto") & (clientes["DataPagamento"] < hoje)]
        a_vencer = clientes[(clientes["Status"] == "Aberto") & (clientes["DataPagamento"] >= hoje) & (clientes["DataPagamento"] <= hoje + timedelta(days=3))]

        if not vencidas.empty:
            st.error("⚠️ Dívidas vencidas")
            st.table(vencidas[["Cliente","Produto","Valor","DataPagamento"]])

        if not a_vencer.empty:
            st.warning("⏳ Dívidas a vencer (próximos 3 dias)")
            st.table(a_vencer[["Cliente","Produto","Valor","DataPagamento"]])
