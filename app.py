...
def show_logo_places():
    ...
    if st.sidebar.button("🔄 Sincronizar agora"):
        try:
            upload_csv_to_drive("produtos.csv", FOLDER_FOTOS)
            upload_csv_to_drive("vendas.csv", FOLDER_VENDAS)
            upload_csv_to_drive("clientes.csv", FOLDER_CLIENTES)
            upload_csv_to_drive("promocoes.csv", FOLDER_PROMOCOES)
            st.sidebar.success("✅ Dados sincronizados com o Google Drive!")
        except Exception as e:
            st.sidebar.error(f"Erro ao sincronizar: {e}")

            st.image(logo, use_column_width=True)
    # Para a tela de login, mostramos no corpo (feito dentro do bloco do login)
