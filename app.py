...
view = st.sidebar.radio("Navegar", ["Dashboard","Produtos","Vendas","Fluxo de Caixa","Promoções"], index=0)
...
# ========================= ABA CLIENTES =========================
    ...
    st.header("👥 Clientes - Vendas Fiado")
    ...
    with st.form("form_clientes"):
        ...
        valor = st.number_input("Valor da Dívida", min_value=0.0, step=0.01, format="%.2f")
    ...
    st.subheader("📋 Lista de Clientes (Fiado)")
    st.dataframe(clientes)

    if not clientes.empty:
        hoje = datetime.date.today()
        clientes["DataPagamento"] = pd.to_datetime(clientes["DataPagamento"]).dt.date

        vencidas = clientes[clientes["DataPagamento"] < hoje]
        a_vencer = clientes[(clientes["DataPagamento"] >= hoje) & (clientes["DataPagamento"] <= hoje + datetime.timedelta(days=3))]
        ...
        if not a_vencer.empty:
            st.warning("📅 Dívidas a Vencer (até 3 dias):")
