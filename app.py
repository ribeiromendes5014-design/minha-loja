...
def garantir_colunas_produtos(df):
    colunas = ["ID", "Nome", "Categoria", "Preço", "Estoque", "Imagem"]
    for c in colunas:
...
def garantir_colunas_vendas(df):
    colunas = ["ID", "Produto", "Quantidade", "Valor", "Data", "Forma de Pagamento"]
    for c in colunas:
        if c not in df.columns:
            df[c] = ""
    return df
...
def garantir_colunas_promocoes(df):
    colunas = ["ID", "Produto", "Desconto", "Validade"]
    for c in colunas:
        if c not in df.columns:
            df[c] = ""
    return df
...
# =============================
if view == "Dashboard":
    # Recarrega dados do Drive para garantir informações atualizadas
...
# =============================
    ...
    if not vendas.empty:
        ...
        if st.button("Excluir venda selecionada"):
            try:
                confirmar = st.checkbox("Confirmar exclusão da venda selecionada")
                if confirmar:
...
def upload_csv_to_drive(df, filename):
    try:
        ...
        # Salva novo somente se tiver dados
        if df is not None and not df.empty:
            tempfile = filename
            df.to_csv(tempfile, index=False)
            media = MediaFileUpload(tempfile, mimetype="text/csv")
            metadata = {"name": filename, "parents": [FOLDER_FOTOS]}
            service.files().create(body=metadata, media_body=media, fields="id").execute()
            os.remove(tempfile)
    except Exception as e:
        st.warning(f"Falha ao enviar {filename} para o Drive: {e}")
