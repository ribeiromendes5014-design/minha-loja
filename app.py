...
# =============================
    ...
    if not abertas.empty:
            opts = [f"{idx} - {row.Cliente} - {row.Produto} - R$ {row.Valor:.2f} - {row.DataPagamento}" for idx, row in abertas.iterrows()]
            escolha = st.selectbox("Selecione uma dívida", opts)
            if escolha:
                idx = int(escolha.split(" - ")[0])
                forma_pg = st.selectbox("Forma de Pagamento (Quitar)", ["Dinheiro","Pix","Cartão"])
                if st.button("💰 Marcar como Pago"):
                    ...
                    if forma_pg in ("Dinheiro","Pix"):
                        ...
                        fator_cartao = FATOR_CARTAO
                        valor_pago = valor_original * fator_cartao
                    clientes.loc[idx, "Status"] = "Pago"
                    salvar_csv(clientes, ARQ_CLIENTES)
                    st.success(f"Dívida quitada com {forma_pg}! Valor pago: R$ {valor_pago:.2f}")

        # Alertas (considerando apenas abertas)
    hoje = date.today()
    ...
    if not base_alerta.empty:
        ...
        base_alerta["DataPagamento"] = pd.to_datetime([], errors="coerce")  # dataframe vazio, inicialização segura
