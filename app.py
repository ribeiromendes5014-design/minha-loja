...
def github_put_file(path:str, content_bytes:bytes, message:str):
    conf = _gh_conf()
    if not conf:
        # Se GitHub não estiver configurado, salva localmente e segue
        try:
...
def salvar_csv_github(df: pd.DataFrame, filename: str):
    ...
    # Sempre grava localmente também
    try:
...
def upload_foto_produto_github(file_bytes:bytes, nome_produto:str, original_name:str):
    ...
    conf = _gh_conf()
    if conf:
        raw_url = f"https://raw.githubusercontent.com/{conf['repo']}/{conf['branch']}/{path}"
    else:
        raw_url = path  # fallback local
...
def show_logo_places():
    ...
    with st.sidebar:
        ...
        # Botão de sincronização manual com o Drive
...
def _load_usuarios_df():
    try:
        ...
        # normalização para evitar espaços/letras maiúsculas
...
def garantir_colunas_produtos(df: pd.DataFrame) -> pd.DataFrame:
    cols = ["ID","Nome","Marca","Categoria","Quantidade",
            "PrecoCusto","PrecoVista","PrecoCartao","Validade",
            "FotoID","FotoURL"]
...
if view == "Dashboard":
    ...
    if not produtos.empty:
        ...
        produtos["Validade_dt"] = pd.to_datetime(produtos["Validade"], errors="coerce")
        vencendo = produtos[produtos["Validade_dt"].between(hoje, limite)]
...
# Tabela com os dados básicos
st.dataframe(produtos[["ID","Nome","Marca","Categoria","Quantidade","PrecoCusto","PrecoVista","PrecoCartao","Validade"]].sort_values("Nome"),
             use_container_width=True)

# Miniaturas das fotos (quando houver FotoURL)
if "FotoURL" in produtos.columns and produtos["FotoURL"].fillna("").str.len().sum() > 0:
    st.markdown("**Fotos dos produtos**")
    linhas = list(produtos.itertuples(index=False))
    cols = st.columns(5)
    i = 0
    for row in linhas:
        url = ""
        ...
        if url:
            with cols[i % 5]:
                st.image(url, caption=row.Nome, use_container_width=True)
            i += 1
...
if view == "Clientes":
    ...
    with st.expander("Baixar/Pagar conta"):
        ...
        if id_sel is not None:
            forma_pag = st.selectbox("Forma de pagamento", ["Dinheiro","PIX","Cartão"])
        if st.button("Marcar como Pago") and id_sel is not None:
            valor_original = clientes.loc[clientes["ID"]==id_sel, "Valor"].iloc[0]
            if forma_pag == "Cartão":
                valor_recebido = round(valor_original / FATOR_CARTAO, 2)
            else:
                valor_recebido = valor_original
            clientes.loc[clientes["ID"]==id_sel, "Status"] = f"Pago ({forma_pag}) R$ {valor_recebido:,.2f}".replace(",", "X").replace(".", ",").replace("X",".")
            st.session_state.clientes = clientes
            salvar_csv_github(clientes, ARQ_CLIENTES)
            st.success("Lançamento atualizado.")

    with st.expander("Excluir fiado"):
        ids = clientes["ID"].tolist() if not clientes.empty else [] 
        id_exc = st.selectbox("ID do fiado para excluir", ids) if ids else None
        if st.button("Excluir fiado") and id_exc is not None:
            clientes = clientes[clientes["ID"] != id_exc].reset_index(drop=True)
            st.session_state.clientes = clientes
            salvar_csv_github(clientes, ARQ_CLIENTES)
            st.success("Fiado excluído.")
