...
def gerar_recibo_pdf(venda: dict, arquivo: str = "recibo_venda.pdf") -> str:
    """
    Gera um PDF de recibo seguindo o layout do modelo enviado.
    ...
    # Estilos customizados (centralizados como no modelo)
    titulo = ParagraphStyle(name="Titulo", parent=styles["Title"], alignment=1, fontSize=16, spaceAfter=6)
    subtitulo = ParagraphStyle(name="Subtitulo", parent=styles["Normal"], alignment=1, fontSize=10, spaceAfter=2)
    corpo = ParagraphStyle(name="Corpo", parent=styles["Normal"], alignment=0, fontSize=11, spaceAfter=6)
    rodape = ParagraphStyle(name="Rodape", parent=styles["Normal"], alignment=1, fontSize=9, textColor=colors.grey)

    # Logo e cabeçalho
    # Usa o arquivo de logo que você enviou junto do app
    logo_candidates = [
        "ChatGPT Image 1_09_2025, 21_52_48.png",
        os.path.join(os.getcwd(), "ChatGPT Image 1_09_2025, 21_52_48.png"),
    ]
    for logo in logo_candidates:
        if os.path.exists(logo):
            try:
                story.append(Image(logo, width=40*mm, height=20*mm))
            except Exception:
                pass
            break

    story.append(Paragraph("Doce & Bella Cosmético", titulo))
    story.append(Paragraph("Telefone: (41) 99168-6525", subtitulo))
    story.append(Paragraph("Instagram: @docebellacosmetico", subtitulo))
    story.append(Spacer(1, 6))

    story.append(Paragraph("RECIBO DE VENDA", titulo))
    story.append(Spacer(1, 6))

    # Dados da venda (data e forma pagamento)
    story.append(Paragraph(f"<b>Data:</b> {venda['Data']}", corpo))
    story.append(Paragraph(f"<b>Forma de Pagamento:</b> {venda['FormaPagamento']}", corpo))
    story.append(Spacer(1, 4))

    # Divisor
    story.append(Paragraph("<hr width='100%'/>>", corpo))
    ...
    tabela = Table(
        [
            ...
            [venda["NomeProduto"], venda["Quantidade"],
             f"R$ {float(venda['PrecoUnitario']):.2f}", f"R$ {float(venda['Total']):.2f}"]
        ],
    ...
    # Total em destaque
    story.append(Paragraph(f"<b>Valor Total: R$ {float(venda['Total']):.2f}</b>", corpo))
    story.append(Spacer(1, 12))

    story.append(Paragraph("Obrigado pela sua compra, volte sempre!", corpo))
    story.append(Spacer(1, 16))

    # Rodapé com data/hora
    agora = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    story.append(Paragraph(f"Documento gerado em {agora}", rodape))
...
# =============================
    ...
    if produtos.empty:
        ...
        if tem_promo and promo_txt is not None:
            st.info(promo_txt)
        ...
        if st.button("Registrar Venda"):
            if int(qtd) > int(p["Quantidade"]):
                ...
                # Gerar recibo PDF
                arquivo_recibo = gerar_recibo_pdf(nova, "recibo_venda.pdf")

                # Botão de download do recibo
                with open(arquivo_recibo, "rb") as f:
                    st.download_button("📥 Baixar Recibo", f, file_name="recibo_venda.pdf", mime="application/pdf")
...
# =============================
    ...
    if "Status" not in clientes.columns:
        clientes["Status"] = "Aberto"
    abertas = clientes[clientes.get("Status", "Aberto") != "Pago"].copy()
